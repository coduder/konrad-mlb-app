...
export class MlbGameService {
  ...
  private makeTeam(boxscore, homeAway: string) {
    ...
    return new Team(
      ...
      (homeAway === 'home') ? boxscore['batting'][0]['batter'] : boxscore['batting'][1]['batter']
      // ^^ the api has a 'batting' array with 2 indexes, the first matches to home and 2nd matches to away
    );
  }
